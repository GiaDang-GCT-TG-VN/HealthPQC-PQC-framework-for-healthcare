...
> **Apple Silicon users: read `docs/liboqs_install.md` first.**  
> Standard `pip install liboqs` does not work on ARM64. The install guide covers the exact 5-block cmake build procedure, the ARM64 architecture flag, and a verification script.

```bash
# 1. Clone the repository
git clone https://github.com/YOUR_USERNAME/pqc-healthcare-framework
cd pqc-healthcare-framework

# 2. Install liboqs (Apple Silicon — build from source)
# Follow docs/liboqs_install.md exactly — all 5 blocks in order

# 3. Verify liboqs works before anything else
python verify_liboqs.py
# Expected: "liboqs is fully operational. Ready for Phase 1."
...
> Generated by `benchmarks/healthcare_benchmark.py` — real numbers from ibm_brisbane-comparable hardware

| Algorithm | FIPS | Use Case | KeyGen (ms) | PubKey (B) | Output (B) | Note |
|-----------|------|----------|-------------|------------|------------|------|
| ML-KEM-768 | 203 | EHR TLS session | 0.031 | 1184 | 1088 | NIST L3 — standard choice |
| ML-KEM-1024 | 203 | Long-term patient data | 0.044 | 1568 | 1568 | NIST L5 — 10yr retention |
| ML-DSA-65 | 204 | Clinical code signing | 0.089 | 1952 | 3309 | NIST L3 — server-side |
| Falcon-512 | 206 | Medical IoT auth | 9.2 | 897 | ~666 | **IoT choice — 5× smaller sig** |
| Hybrid (X25519 + ML-KEM-768) | — | External TLS endpoints | +0.04 | — | — | Negligible overhead |

**P99 latency delta (hybrid vs classical at 1000 concurrent users): < 2ms**
...
├── verify_liboqs.py                   ← run this first — confirms liboqs works on ARM64
├── requirements.txt
├── Dockerfile
├── docker-compose.yml
│
...
│
└── docs/
    ├── liboqs_install.md              ← START HERE — Apple Silicon ARM64 install guide
