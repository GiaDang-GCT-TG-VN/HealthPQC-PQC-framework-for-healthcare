...
- Python 3.13 virtual environment (`my_qiskitenv`) active
- Homebrew installed (`brew --version` works)
- Apple Silicon (M1/M2/M3) — confirmed with `uname -m` → `arm64`
- At least 2GB free disk space

> **Full installation guide**: `docs/liboqs_install.md`  
> Read that file completely before running any commands here.  
> It covers the exact 5-block procedure, the ARM64 architecture gotcha, a full troubleshooting table, and a verification script to confirm the library is working correctly.

### Why standard pip install fails on Apple Silicon

```
pip install liboqs        ← this does NOT work on ARM64
```

The PyPI wheel was compiled for x86_64 (Intel). Apple Silicon is ARM64. The architectures are incompatible — the wheel either fails to import or runs through Rosetta 2 emulation with broken cryptographic results.

The fix is always: **build liboqs from source**, explicitly targeting ARM64. The full procedure is in `docs/liboqs_install.md`.

### FIPS quick reference — memorise before starting

```
FIPS 203 = ML-KEM (Kyber)    — key exchange, replaces RSA/ECDH in TLS
FIPS 204 = ML-DSA (Dilithium) — signatures, replaces ECDSA in PKI
FIPS 206 = FN-DSA (Falcon)   — compact signatures, replaces ECDSA on IoT
```

**Crypto-agility** = ability to swap crypto algorithms without rebuilding the system.  
Demonstrate it with `config/crypto_config.yaml` → change one line, algorithm swaps everywhere.

### Installation summary (5 blocks — full detail in `docs/liboqs_install.md`)

```bash
# Block 1 — Prerequisites
brew install cmake ninja

# Block 2 — Activate venv
cd ~/my_qiskitenv && source bin/activate

# Block 3 — Build liboqs C library (ARM64 explicit)
...
# Block 4 — Python bindings
pip install git+https://github.com/open-quantum-safe/liboqs-python

# Block 5 — Verify
python -c "
import oqs
print('ML-KEM-768:', 'ML-KEM-768' in oqs.get_enabled_KEMs())
print('ML-DSA-65: ', 'ML-DSA-65'  in oqs.get_enabled_sigs())
print('Falcon-512:', 'Falcon-512' in oqs.get_enabled_sigs())
"
# All three should print True
```

> **If Block 3 fails with architecture mismatch**: the `-DCMAKE_OSX_ARCHITECTURES=arm64` flag is the fix. Delete `~/liboqs/build` and re-run cmake with that flag. Full error table in `docs/liboqs_install.md`.

### Verification script (run after Block 5)

```bash
python verify_liboqs.py
```

This script (in the repo root) tests an actual ML-KEM-768 key exchange and a Falcon-512 signature — confirming not just that the library imports but that the algorithms execute correctly. Expected output: `liboqs is fully operational. Ready for Phase 1.`

Only proceed to Deliverable 1.1 after this script passes.
